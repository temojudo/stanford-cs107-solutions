...
hashset stopWords, words;
int num;

void freeVector(void * elemAddr) {
  artic * elem = elemAddr;
  free(elem->articleName);
  free(elem->URL);
}

void freeSimple(void * elemAddr) {
  free(*(char **)elemAddr);
}

void freeFn(void * elemAddr) {
  Map * elem = elemAddr;
  VectorDispose(&elem->articles);
  free(elem->word);
}
...
static int StringHash(const void * a, int numBuckets) {
  ...
  for (int i = 0; i < strlen(s); i++) hashcode = hashcode * kHashMultiplier + tolower(s[i]);  
  return hashcode % numBuckets;                                
}

static int compareFunction(const void * b, const void * a) {
  return strcasecmp(*(char **) a, *(char **) b);
}

static int vectorCMP(const void * b, const void * a) {
  artic * first = (artic *)a;
  artic * second = (artic *)b;

  if (first->times > second->times) return 1;
  if (first->times < second->times) return -1;
  
  if (strcasecmp(first->articleName, second->articleName) > 0) return 1;
  if (strcasecmp(first->articleName, second->articleName) < 0) return -1;
...
static int compareSpecialFunction(const void * b, const void * a) {
  artic * first = (artic *)a;
  artic * second = (artic *)b;
...
void mapFn(void * element, void * auxdata) {
  int * count = auxdata;
  artic * elem = element; 
  
  printf("%d.) \"%s\" [search term occurs %d ", *count, elem->articleName, elem->times);
  if (elem->times > 1) printf("times]");
                  else printf("time]");
...
static void createStopWords(char * filename) {
  streamtokenizer st;
  FILE * f = fopen(filename, "r");
  STNew(&st, f, kNewLineDelimiters, true);

  char buffer[1024];
  while (STNextToken(&st, buffer, sizeof(buffer))) {
    char * dest = strdup(buffer);
    HashSetEnter(&stopWords, &dest);
  }
...
int main(int argc, char **argv) {
  HashSetNew(&words, sizeof(Map), 2001, StringHash, compareFunction, freeFn);
  HashSetNew(&stopWords, sizeof(char *), 2003, StringHash, compareFunction, freeSimple);
  createStopWords("data/stop-words.txt");
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *unused, const char *articleURL)
{
  char * art = strdup(articleTitle);
  if (HashSetLookup(&stopWords, &art)) {
    free(art);
    return;
  }
  ...
  while (STNextToken(st, word, sizeof(word))) {
    ...
    if (WordIsWellFormed(word)) {
      char * wordd = strdup(word);
      if (HashSetLookup(&stopWords, &wordd)) { free(wordd); numWords++; continue; }
      Map * curr = HashSetLookup(&words, &wordd);
      free(wordd);
      if (curr == NULL) {
        Map ell;
        ell.word = strdup(word);
        
        VectorNew(&ell.articles, sizeof(artic), freeVector, 0);
        artic new;
        new.articleName = strdup(articleTitle);
        new.URL = strdup(articleURL);
        new.times = 1;
        VectorAppend(&ell.articles, &new);
        HashSetEnter(&words, &ell);
      } else {
        artic go;
        go.articleName = strdup(articleTitle);
        go.URL = strdup(articleURL);
        int index = VectorSearch(&curr->articles, &go, compareSpecialFunction, 0, false);
        free(go.articleName);
        free(go.URL);

        if (index < 0) {
          artic toAdd;
          toAdd.articleName = strdup(articleTitle);
          toAdd.URL = strdup(articleURL);
          toAdd.times = 1;
          VectorAppend(&curr->articles, &toAdd);
        } else {
          artic * add = VectorNth(&curr->articles, index);
          add->times++;
        }
      }
    	numWords++;
...
static void ProcessResponse(const char *word)
{
  char * wordd = strdup(word);
  if (WordIsWellFormed(word)) {
    if (HashSetLookup(&stopWords, &wordd) != NULL) {
    ...
    VectorSort(&curr->articles, vectorCMP);
    num = 1;
    VectorMap(&curr->articles, mapFn, &num);
    free(wordd);
