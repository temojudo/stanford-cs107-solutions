...
void doSomething(Definition &def, vector<string> &ans, map<string, Definition> &grammar) {
  Production p = def.getRandomProduction();
  for (Production::iterator it = p.begin(); it != p.end(); it++) {
    string word = *it;
    if (word[0] == '<' && word[word.length() - 1] == '>' && grammar.find(word) == grammar.end()) exit(0);
    if (grammar.find(word) != grammar.end()) doSomething(grammar[word], ans, grammar);
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i <= 3; i++) {
    ...
    doSomething(grammar["<start>"], ans, grammar);
