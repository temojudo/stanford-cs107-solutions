             ...
             AccountAmount initialAmount)
{
  ...
  sem_init(&(account->adjustLock), 0, 1);
  sem_init(&(account->balLock), 0, 1);
...
{
  sem_wait(&(account->balLock));
  AccountAmount balance = account->balance; Y;
  sem_post(&(account->balLock));
  return balance;
}
